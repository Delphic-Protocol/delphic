...
| `delphic-close`   | [`closePosition.ts`](delphic-cre/delphic-cre/closePosition.ts) | `ClosePositionRequested` log event | Decodes pre-signed SELL order + CLOB credentials from the event, relays the SELL to Polymarket CLOB, writes order hash back to `PositionManager`                                                  |
